...
Window_BattleStatus.prototype.drawBasicArea = function(rect, actor) {
    ...
    this.drawActorName(actor, rect.x + iw + 4, 145, rect.width);
};
...
Window_BattleStatus.prototype.drawGaugeArea = function(rect, actor) {
    ...
    var wymod = Math.max(30, wymod);
    this.drawActorHp(actor, rect.x, 180 - wymod, rect.width);
    if (this.getGaugesDrawn(actor) <= 2) {
      this.drawActorMp(actor, rect.x, 180, rect.width);
    } else {
      var ww = rect.width / 2;
...
Window_BattleStatus.prototype.drawStatusFace = function(index) {
    ...
    var ww = Math.min(rect.width - 0, Window_Base._faceWidth);
    var wh = Math.min(rect.height - 0, Window_Base._faceHeight);
    var wx = rect.x + rect.width - ww - 0;
    var wy = rect.y + 15;
